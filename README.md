...
- ✅ **真实台账追加**：Excel 按设备主人匹配或新建分表，在最新空白行追加并保留原表格式
- ✅ **Excel 模板迭代**：界面可上传最新版 `.xlsx`，默认以上一次生成结果为下一次模板连续追加
- ✅ **Word 模板填充**：替换 `{{占位符}}`（正文和表格都支持）、插入铭牌图片、插入附件图片
- ✅ **工程动向现场选项**：安全告知、平面图和配合程度三项选择自动写入 Word 工程动向正文
...
3. 「②～⑥ 填写信息」：核对自动填入的内容，补充工程动向、三个现场选项、蓝色区域和 Excel 字段
4. 「⑥ Excel 补充信息」：如有新版台账，可上传或拖入 `.xlsx`；默认勾选连续追加模式
5. 「⑦ 上传附件」：选择或直接拖入盖章扫描件等附件（会插入 Word 日期行后的“附件”位置）
6. 「⑧ 生成文件」：选择输出目录后，程序会新建“日期-线路名称-设备主人名字”文件夹，再生成 Word 和 Excel

## 当前真实模板模式

当前版本已接入真实模板：Word 按标签文字和段落位置自动填充，模板中的黄色/绿色/蓝绿色只是辅助标记，生成成品会清除底色。右侧“工程动向”及安全告知、平面图、配合程度三个选项会写入对应正文段落；这三个选项不写入 Excel。铭牌照片和上传/拖入的附件会按顺序放到日期行后。Excel 按设备主人匹配分表，在已有数据最后一行后追加并沿用上一行格式；界面可上传新版模板，且默认以上一次生成的 Excel 为下一次模板继续迭代。
...
**Q9：新的 Excel 模板怎样投入使用？**
A：在程序“Excel 补充信息”中的模板区域上传或拖入 `.xlsx` 即可，不必替换程序文件。上传后旧的连续追加链会清空，下一次从新模板开始；之后程序自动沿用每次生成的最新 Excel。
