...
## 十五、✅ 2026-09-05 工程动向选项与 Excel 模板连续迭代

- 工程动向下方新增三个下拉字段：“是否安全告知”（是/否/留置送达）、“施工方是否提供平面图”（是/否）、“预估施工方配合程度”（积极/一般/消极）。
- 三个字段只追加到 Word 的 `c.工程动向` 正文，不写入 Excel；未选择的字段不会在 Word 中生成空标签。
- Excel 补充区域新增 `.xlsx` 模板上传/拖入入口、当前模板来源提示和“恢复内置模板”按钮；上传文件会先进行工作簿有效性检查。
- 默认启用连续迭代：首次使用当前基础模板，生成成功后自动把该次 Excel 记为下一次模板；程序重启后仍会继续，若上次文件被移动或删除则回退到上传模板或内置模板。
- Excel 双层表头匹配增加空白符/换行归一化，模板表头轻微换行调整后仍可匹配原字段。
- 新增 `测试/test_excel_template_iteration.py`，覆盖上传、两次连续追加、恢复内置模板、换行表头匹配，以及三个新字段不进入 Excel 的规则；Word 成品已实际渲染检查。
- Windows 目录版已完成冻结程序端到端回归，交付包为 `outputs\\一键流转单-目录版-20260905-Excel迭代版.zip`（约 297 MB）；ZIP 完整性检查通过，不含测试模板和运行状态文件。
